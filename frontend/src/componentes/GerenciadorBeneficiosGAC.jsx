...
const GerenciadorBeneficiosGAC = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="modal-edicao-overlay" onClick={onClose}>
      <div className="modal-edicao-container" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '550px' }}>
        <div className="modal-edicao-header">
          <h2 className="modal-edicao-titulo">⚙️ Gerenciar Tipos de Benefícios</h2>
          <button className="modal-edicao-close" onClick={onClose} type="button" disabled={carregando}>
            <X size={20} />
          </button>
        </div>

        <div className="modal-edicao-content" style={{ maxHeight: '600px' }}>
