...
async function obterToken() {
  ...
  if (!EMAIL || !SENHA) { log('Nenhum token ou credenciais fornecidas. Exporte TEST_TOKEN ou TEST_USER_EMAIL & TEST_USER_SENHA'); return null; }
  try { const res = await axios.post(`${BASE_URL}/autenticacao/entrar`, { email: EMAIL, senha: SENHA }); return res.data.token; } catch (err) { log('Erro ao autenticar:', err.response?.data || err.message); return null; }
}
...
async function run() {
  ...
  try {
    ...
    if (!alunos.length) { log('Nenhum aluno encontrado para testar. Pare o script e crie um aluno antes de rodar o teste.'); return; }
    const aluno = alunos[0];
    const payload = { alunoId: aluno.id, ano: TEST_ANO || new Date().getFullYear(), tipo: 'MATRICULA', nomeEscola: 'Escola Teste', horarioEstudo: 'Manhã', horaEntrada: '07:30', horaSaida: '12:00' };
  ...
  } catch (err) { log('Erro durante o teste:', err.response?.data || err.message); }
}
